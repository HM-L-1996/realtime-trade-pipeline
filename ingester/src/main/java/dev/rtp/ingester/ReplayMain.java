 ...
 *
 * <h2>소스</h2>
 * ClickHouse {@code trades_raw} 가 아니라 Kafka 를 읽는다.
 *
 * <p><b>처음 이유는 "그 테이블을 채우는 주체가 아직 없어서" 였는데 그건 이제 낡았다</b> -
 * 집계 잡이 {@code --archive-trades} 로 채우고 있다. 지금 이유는 다르다.
 * {@code trades_raw} 는 <b>보존 기간이 30일이고 재처리 경로가 아니다</b>(증거 보존용).
 * 리플레이의 상한은 Kafka 보존(7일)과 같아야 "되감아 다시 처리한다" 는 의미가 유지된다.
 * 30일치를 리플레이 소스로 쓰려면 별도 작업이 필요하고 지금은 하지 않았다 -
 * {@code docs/failure-policy.md} 의 "되메우기 정책" 참고.
 *
 * <h2>이벤트타임 재기준</h2>
